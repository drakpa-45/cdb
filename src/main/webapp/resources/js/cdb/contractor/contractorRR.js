...
function submitApplication(){
    ...
    cdbGlobal.formIndexing($('#certificateFirmNameTbl').find('tbody'));
...
var contractorRC = (function () {
    ...
    var certFN = "<tr><td></td>" +
        "<td><input type='hidden' class='form-control aFor' name='firmNameAttachments[0].attachmentFor' value='FN'/>" +
        "<input type='text' class='form-control' name='firmNameAttachments[0].documentName'/> </td>"+
        "<td><input type='file' name='firmNameAttachments[0].attachment' class='form-control-file file' accept='application/msword,application/pdf,application/vnd.ms-excel,image/gif, image/jpeg, image/jpg,application/vnd.openxmlformats-officedocument.wordprocessingml.document'></td>" +
        "<td class='file-size'></td>" +
        "<td><a class='p-2 del_row'><i class='fa fa-trash text-danger'></i></a></td>" +
        "</tr>";
    ...
    function addMoreCertFirmName(){
        $('#addMoreCertFirmName').on('click',function(e){
            var certificateTbl = $('#certificateFirmNameTbl').find('tbody').append(certFN);
            /*var row = certificateTbl.find('tr:eq(0)').html();
             certificateTbl.append('<tr>'+row.find(':input').val('')+'</tr>');*/
        });
    }
    ...
    function service_check(){
        $('body').on('click','.service_check',function(){
            ...
            if(id == 'Incorporation' ){
                ...
                if($this.is(':checked')) {
                    $('#firmName').prop('disabled', false).prop('required', true);
                    $('#cFirmName').removeClass('hidden');
                }else{
                    $('#firmName').prop('disabled', true);
                    $('#cFirmName').addClass('hidden');
                }
            ...
            success: function (data) {
                if(data){
                    ...
                    for(var i in data){
                        ...
                        if(data[i].attachmentFor == 'FN'){
                            fNcTr = fNcTr + "<tr>" +
                            "<td></td>" +
                            "<td>" +
                            "<input type='hidden' class='form-control aFor' name='firmNameAttachments[0].attachmentFor' value='FN'/>" +
                            "<input type='hidden' class='form-control' name='firmNameAttachments[0].id' value='"+data[i].id+"'/>" +
                            "<input type='text' class='form-control docName' name='firmNameAttachments[0].documentName' value='"+data[i].documentName+"'/ disabled></td>" +
                            "<td class='attachmentfn'><a href='" + _baseURL() + "/viewDownload?documentPath=" + data[i].documentPath + "' target='_blank'> View </a></td>" +
                            "<td></td>" +
                            "<td class='action'><button class='btn-sm btn-info btn-block edit_row_fn' >Edit</button>" +
                            "<button class='btn-sm btn-info btn-block del_row'>Delete</button></td>" +
                            "</tr>";
                        }
                    }
                    ...
                    $('#certificateFirmNameTbl').find('tbody').html(fNcTr);
                }else{
    ...
    function editFNAttachment(){
        $('#certificateFirmNameTbl').on('click','.edit_row_fn',function(){
            $(this).closest('tr').find('.docName').prop('disabled',false);
            var attachment = $(this).closest('tr').find('.attachmentfn');
            attachment.html("<input type='file' name='ownerAttachments[0].attachment' class='form-control-file file' accept='application/msword,application/pdf,application/vnd.ms-excel,image/gif, image/jpeg, image/jpg,application/vnd.openxmlformats-officedocument.wordprocessingml.document'>");
        })
    }
                    ...
                    success: function (res) {
                        ...
                        for (var i in categories) {
                            if (categories[i].categoryId == "6cd737d4-a2b7-11e4-b4d2-080027dcfac6") {
                                ...
                                $('#W1').find('.appliedClassID').val(categories[i].aClassId).prop('disabled', false);
                            ...
                            if (categories[i].categoryId == "8176bd2d-a2b7-11e4-b4d2-080027dcfac6") {
                                ...
                                $('#W2').find('.appliedClassID').val(categories[i].aClassId).prop('disabled', false);
                            ...
                            if (categories[i].categoryId == "8afc0568-a2b7-11e4-b4d2-080027dcfac6") {
                                ...
                                $('#W3').find('.appliedClassID').val(categories[i].aClassId).prop('disabled', false);
                            ...
                            if (categories[i].categoryId == "9090a82a-a2b7-11e4-b4d2-080027dcfac6") {
                                ...
                                $('#W4').find('.appliedClassID').val(categories[i].aClassId).prop('disabled', false);
    ...
    function editInModalOwner(){
        $('body').on('click','.edit-rowOW',function(e){
            ...
            hrModal.find('#ow1').val(hrModal.find('#ow1 option:contains("'+row.find('.countryName').text()+'")').val()).prop('disabled',true);
            hrModal.find('#ow2').val(row.find('.cidNo').text()).prop('disabled',true);
            hrModal.find('#ow3').val(hrModal.find('#ow3 option:contains("'+row.find('.salutationName').text()+'")').val()).prop('disabled',true);
            hrModal.find('#ow4').val(row.find('.name').text()).prop('disabled',true);
            hrModal.find('#ow5').val(row.find('.sex').text()).prop('disabled',true);
    ...
    function editInModalEQ(){
        $('#equipmentTbl').on('click','.edit_row_eq',function(e){
            ...
            modal.find('#eq1').val(modal.find('#eq1 option:contains("' + row.find('td:nth-child(1)').text() + '")').val()).prop('disabled',true);
            modal.find('#eq2').val(row.find('td:nth-child(2)').text()).prop('disabled',true);
            modal.find('#eq3').val(row.find('td:nth-child(3)').text()).prop('disabled',true);
            var eqaTr = "";
    ...
    function validate_eq() {
        $('#btnValEqNext').on('click', function (e) {
            ...
            var classIds = '';
            var classML = false;
            $('#contractorCCTbl').find('.appliedClassID').each(function(){
